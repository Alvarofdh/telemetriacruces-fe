...
export function DataProvider({ children }) {
  ...
  const login = (email, password) => {
    ...
    if (usuario && password === 'admin123') {
      setUser(usuario)
      setIsAdmin(['SUPER_ADMIN', 'SUPERVISOR'].includes(usuario.rol))
  ...
  const stats = {
    ...
    promedioBateria: Math.round(cruces.reduce((acc, c) => acc + c.bateria, 0) / cruces.length),
